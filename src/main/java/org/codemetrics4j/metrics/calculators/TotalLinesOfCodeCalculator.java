    ...
    private static NumericValue performCalculation(Stack<Node> nodeStack) {
        ...
        while (!nodeStack.empty()) {
            ...
            if (node instanceof FieldDeclaration) {
                ...
                count = count
                        + 2; // 2 for the opening and closing of the for statement, we ignore complexity within the loop
                // condition itself
                ...
                count = count + 1;
                // nodeStack.add(((ExpressionStmt)node).getExpression()); //TODO: maybe be a little smarter, go into
                // more detail here since there can be so much code in an expression.  same for the statements below?
